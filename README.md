...
New commands:
- `!uptime` - Show for how long the bot has been on (`!upt`)
- `!weight <user>` - Respond with the skyblock weight of the specified user (`!we`, `!weights`, `!poid`, `!poids`)
- `!skycrypt [user]` - Send the skyblock link of the user (`!sc`, `!sb`, `!pv`)

### Others
- Update a discord channel with current online users
- Fragbot
- Automated chat calculator
- Direct access to minecraft chat of the bot
### Roadmap
